...
def upgrade(conn):
    conn.execute(text("""
        CREATE TABLE IF NOT EXISTS market_structure_h1 (
            ...
            UNIQUE KEY uq_h1 (symbol, timeframe, structure_type, previous_swing_high_time, current_swing_high_time, previous_swing_low_time, current_swing_low_time)
        )
    """))

def downgrade(conn):
    conn.execute(text("DROP TABLE IF EXISTS market_structure_h1"))