...
def upgrade(conn):
    conn.execute(text("""
        CREATE TABLE IF NOT EXISTS market_structure_m5 (
            ...
            UNIQUE KEY uq_m5 (symbol, timeframe, structure_type, previous_swing_high_time, current_swing_high_time, previous_swing_low_time, current_swing_low_time)
        )
    """))

def downgrade(conn):
    conn.execute(text("DROP TABLE IF EXISTS market_structure_m5"))